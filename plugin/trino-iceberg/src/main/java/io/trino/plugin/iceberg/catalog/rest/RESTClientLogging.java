...
{
    private static final Logger LOG = Logger.get(RESTClientLogging.class);
        ...
        if (responseBody != null) {
            try {
                if (errorHandler instanceof ErrorHandler) {
                    ...
                    LOG.warn(
                            "Unknown error handler %s, response body won't be parsed",
                ...
                LOG.error("Failed to parse an error response. Will create one instead. %s", e);
            }
        }
            ...
            Object body)
    {
        ...
        LOG.info("Building request: Method=%s, Path=%s, QueryParams=%s, Headers=%s",
                    method, path, queryParams, headers);
        if (body != null) {
            String encoded;
            if (body instanceof Map) {
                encoded = RESTUtil.encodeFormData((Map<?, ?>) body);
            }
            else {
                try {
                    ...
                    encoded = "<failed to serialize body: " + e.getMessage() + ">";
                }
            }
            LOG.info("Request body: %s", encoded);
        }
    ...
    {
        // Log the request details
        LOG.info("Executing request: Method=%s, Path=%s, QueryParams=%s, Headers=%s, ResponseHeaders=%s",
                req.method(), req.path(), req.queryParameters(), req.headers(), responseHeaders);
        ...
        if (encodedBody != null) {
            ...
            LOG.info("Request body: %s", encodedBody);
        }
        ...
        try (CloseableHttpResponse response = httpClient.execute(request)) {
            ...
            try {
                LOG.info("responseType=%s, responseBody=%s", responseType.getSimpleName(), responseBody);
    ...
    public static class Builder
    {
        ...
        public RESTClientLogging build()
        {
