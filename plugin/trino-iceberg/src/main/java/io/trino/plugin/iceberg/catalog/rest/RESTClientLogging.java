        ...
        implements RESTClient
{
            ...
            Consumer<ErrorResponse> errorHandler)
    {
        ...
        LOG.info("Executing request: Method=%s, Path=%s, QueryParams=%s, Headers=%s",
                    method, path, queryParams, headers);
        if (requestBody != null) {
            LOG.info("Request body: %s", requestBody instanceof Map ? requestBody : toJson(requestBody));
        }
        return execute(
            ...
            Consumer<Map<String, String>> responseHeaders)
    {
        ...
        LOG.info("Executing request: Method=%s, Path=%s, QueryParams=%s, Headers=%s, ResponseHeaders=%s",
                method, path, queryParams, headers, responseHeaders);
        if (requestBody != null) {
            LOG.info("Request body: %s", requestBody instanceof Map ? requestBody : toJson(requestBody));
        }
        ...
        try {
            ctor =
                DynConstructors.builder(HttpRequestInterceptor.class)
                    .loader(RESTClientLogging.class.getClassLoader())
                    .impl(impl)
                    .buildChecked();
        }
